...
namespace NovaComponents\NovaNestedForm;
...
class NestedForm extends Field implements RelatableField
{
    ...
    /**
     * The heading template for panel.
     *
     * @var string
     */
    public $panelHeading;
    ...
    {
        if (is_callable($heading)) {
            $this->heading = $heading();
        } else if (is_string($heading)) {
            $this->heading = $heading;
        } else {
            throw new \Exception('Invalid heading type');
        }
