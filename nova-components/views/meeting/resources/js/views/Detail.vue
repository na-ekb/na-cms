  ...
  <loading-view :loading="initialLoading">
    ...
    >
      <div v-if="panel.showToolbar" class="flex items-center mb-3">
        <heading :level="1" class="flex-auto truncate"></heading>
      ...
      <div class="card mb-6 py-3 px-6 flex-wrap flex">
        <div class="flex flex-wrap w-full">
          <div class="w-1/2">
            <div class="flex border-b border-40 flex-col">
              <div class="pt-6 w-full">
                <h4 class="font-normal text-80">Название группы</h4>
              </div>
              <div class="py-4 break-words w-full">
                <p class="text-90">{{ resObj.title }}</p>
              </div>
            </div>
          </div>
          <div class="w-1/2">
            <div class="flex border-b border-40 flex-col">
              <div class="pt-6 w-full">
                <h4 class="font-normal text-80">Тип группы</h4>
              </div>
              <div class="py-4 break-words w-full">
                <p class="text-90">{{ resObj.ComputedField }}</p>
              </div>
            </div>
          </div>
        </div>
        <div class="flex flex-wrap w-full">
          <div class="w-1/4">
            <div class="flex border-b border-40 flex-col">
              <div class="pt-6 w-full">
                <h4 class="font-normal text-80">Город</h4>
              </div>
              <div class="py-4 break-words w-full">
                <p class="text-90">{{ resObj.city }}</p>
              </div>
            </div>
          </div>
          <div class="w-1/4">
            <div class="flex border-b border-40 flex-col">
              <div class="pt-6 w-full">
                <h4 class="font-normal text-80">Район/метро</h4>
              </div>
              <div class="py-4 break-words w-full">
                <p class="text-90">{{ resObj.location }}</p>
              </div>
            </div>
          </div>
          <div class="w-1/4">
            <div class="flex border-b border-40 flex-col">
              <div class="pt-6 w-full">
                <h4 class="font-normal text-80">Адрес</h4>
              </div>
              <div class="py-4 break-words w-full">
                <p class="text-90">{{ resObj.address }}</p>
              </div>
            </div>
          </div>
          <div class="w-1/4">
            <div class="flex border-b border-40 flex-col">
              <div class="pt-6 w-full">
                <h4 class="font-normal text-80">Примечание</h4>
              </div>
              <div class="py-4 break-words w-full">
                <p class="text-90">{{ resObj.address_description }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        .then(({ data: { title, panels, resource } }) => {
          ...
          for (let i = 0; i < resource.fields.length; i++) {
            if (resource.fields[i].attribute == '' && typeof resource.fields[i].fields != 'undefined') {
              for (let n = 0; n < resource.fields[i].fields.length; n++) {
                this.resObj[resource.fields[i].fields[n].attribute] = resource.fields[i].fields[n].value;
              }
            } else {
              this.resObj[resource.fields[i].attribute] = resource.fields[i].value;
            }
          }
        })
        .catch(error => {
