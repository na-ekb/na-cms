@if (count(\Laravel\Nova\Nova::resourcesForNavigation(request())))
    <h3 class="flex items-center font-normal text-white mb-6 text-base no-underline">
        ...
        <span class="sidebar-label">{{ __('admin/nova.resources') }}</span>
    </h3>
    @php
        $navigation = $navigation->toArray();
        uksort($navigation, function($a, $b) {
            if (config("nova-group-order.{$a}") == config("nova-group-order.{$b}")) {
                return 0;
            }
            return (config("nova-group-order.{$a}") < config("nova-group-order.{$b}")) ? -1 : 1;
        });
    @endphp
