<?php namespace NAEkb\Pages\Controllers;
...
use NAEkb\Pages\Widgets\PageList;
use NAEkb\Pages\Widgets\MenuList;
use NAEkb\Pages\Widgets\TemplateList;
use NAEkb\Pages\Classes\Page as StaticPage;
use NAEkb\Pages\Classes\Content;
use NAEkb\Pages\Classes\MenuItem;
use NAEkb\Pages\Plugin as PagesPlugin;
...
class Index extends Controller
{
    ...
    public $requiredPermissions = ['naekb.pages.*'];
    ...
    public function __construct()
    {
        ...
        try {
            ...
            if ($this->user) {
                if ($this->user->hasAccess('naekb.pages.manage_pages')) {
                    new PageList($this, 'pageList');
                    $this->vars['activeWidgets'][] = 'pageList';
                }

                if ($this->user->hasAccess('naekb.pages.manage_menus')) {
                    new MenuList($this, 'menuList');
                    $this->vars['activeWidgets'][] = 'menuList';
                }

                if ($this->user->hasAccess('naekb.pages.manage_content')) {
        ...
        BackendMenu::setContext('NAEkb.Pages', 'pages', @$context[$this->vars['activeWidgets'][0]]);
    }
    ...
    {
        $this->addJs('/plugins/naekb/pages/assets/js/october.treeview.js', 'NAEkb.Pages');
        $this->addJs('/plugins/naekb/pages/assets/js/pages-page.js', 'NAEkb.Pages');
        $this->addCss('/plugins/naekb/pages/assets/css/pages.css', 'NAEkb.Pages');
        $this->addCss('/plugins/naekb/pages/assets/css/treeview.css', 'NAEkb.Pages');
        ...
        $this->pageTitle = 'naekb.pages::lang.plugin.name';
        $this->pageTitleTemplate = Lang::get('naekb.pages::lang.page.template_title');
    ...
    public function onSave()
    {
        ...
        $successMessages = [
            'page' => 'naekb.pages::lang.page.saved',
            'menu' => 'naekb.pages::lang.menu.saved',
            'content' => 'naekb.pages::lang.content.saved',
        ];
    ...
    public function onMenuItemReferenceSearch()
    {
        ...
        $widget = $this->makeFormWidget(
            'NAEkb\Pages\FormWidgets\MenuItemSearch',
            [],
            ['alias' => $alias]
        );
    ...
    protected function loadObject($type, $path, $ignoreNotFound = false)
    {
        ...
        if (!($object = call_user_func(array($class, 'load'), $this->theme, $path))) {
            if (!$ignoreNotFound) {
                throw new ApplicationException(trans('naekb.pages::lang.object.not_found'));
            }
    ...
    {
        $types = [
            'page' => \NAEkb\Pages\Classes\Page::class,
            'menu' => \NAEkb\Pages\Classes\Menu::class,
            'content' => \NAEkb\Pages\Classes\Content::class
        ];

        if (!array_key_exists($type, $types)) {
            throw new ApplicationException(Lang::get('naekb.pages::lang.object.invalid_type') . ' - type - ' . $type);
        }

        $allowed = false;
        if ($type === 'content') {
            $allowed = $this->user->hasAccess('naekb.pages.manage_content');
        }
        else {
            $allowed = $this->user->hasAccess("naekb.pages.manage_{$type}s");
        }

        if (!$allowed) {
            throw new ApplicationException(Lang::get('naekb.pages::lang.object.unauthorized_type', ['type' => $type]));
        }
    ...
    {
        $formConfigs = [
            'page'    => '~/plugins/naekb/pages/classes/page/fields.yaml',
            'menu'    => '~/plugins/naekb/pages/classes/menu/fields.yaml',
            'content' => '~/plugins/naekb/pages/classes/content/fields.yaml'
        ];

        if (!array_key_exists($type, $formConfigs)) {
            throw new ApplicationException(Lang::get('naekb.pages::lang.object.not_found'));
        }
    ...
    protected function getTabTitle($type, $object)
    {
        if ($type == 'page') {
            ...
            if (!$result) {
                $result = trans('naekb.pages::lang.page.new');
            }
        ...
        elseif ($type == 'menu') {
            $result = $object->name;
            if (!strlen($result)) {
                $result = trans('naekb.pages::lang.menu.new');
            }
    ...
    protected function fillObjectFromPost($type)
    {
        ...
        if ($type == 'content') {
            ...
            if (dirname($fileName) == 'static-pages') {
                throw new ApplicationException(trans('naekb.pages::lang.content.cant_save_to_dir'));
            }
    ...
    protected function getContentTemplateList()
    {
         ...
         * Or:
         *
         *     \NAEkb\Pages\Controller\Index::extend(function ($controller) {
