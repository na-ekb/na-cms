...
class GroupsCommand extends Command
{
    ...
    public function __construct() {
        $this->description = __('naekb.tgbot::commands.group.description');
    }
    ...
    public function handle(array $arguments = [])
    {
        ...
        $keyboard = [
            [
                Keyboard::button([
                    'text'          => __('naekb.tgbot::commands.group.today'),
                    'callback_data' => "groups  {$arguments['region']} today"
                ]),
                Keyboard::button([
                    'text'          => __('naekb.tgbot::commands.group.tomorrow'),
                    'callback_data' => "groups  {$arguments['region']} tomorrow"
                ]),
                Keyboard::button([
                    'text'          => __('naekb.tgbot::commands.group.after'),
                    'callback_data' => "groups  {$arguments['region']} after"
                ])
            ],
            [
                Keyboard::button([
                    'text'          => __('naekb.tgbot::commands.group.mon'),
                    'callback_data' => "groups  {$arguments['region']} 1"
                ]),
                Keyboard::button([
                    'text'          => __('naekb.tgbot::commands.group.tue'),
                    'callback_data' => "groups  {$arguments['region']} 2"
                ]),
                Keyboard::button([
                    'text'          => __('naekb.tgbot::commands.group.wed'),
                    'callback_data' => "groups  {$arguments['region']} 3"
                ]),
                Keyboard::button([
                    'text'          => __('naekb.tgbot::commands.group.thu'),
                    'callback_data' => "groups  {$arguments['region']} 4"
                ]),
                Keyboard::button([
                    'text'          => __('naekb.tgbot::commands.group.fri'),
                    'callback_data' => "groups  {$arguments['region']} 5"
                ]),
                Keyboard::button([
                    'text'          => __('naekb.tgbot::commands.group.sat'),
                    'callback_data' => "groups  {$arguments['region']} 6"
                ]),
                Keyboard::button([
                    'text'          => __('naekb.tgbot::commands.group.sun'),
                    'callback_data' => "groups  {$arguments['region']} 0"
                ])
            ],
            [
                Keyboard::button([
                    'text'          => __('naekb.tgbot::commands.group.geo'),
        ...
        if ($arguments['type'] !== null) {
            ...
            if (is_numeric($arguments['type'])) {
                ...
                switch ($arguments['type']) {
                    case 'today':
                        $text = __('naekb.tgbot::commands.group.today') . ' (' . Carbon::now()->format('d.m.Y H:i') . ')' . PHP_EOL . PHP_EOL;
                        $day = Carbon::now()->dayOfWeek;
                        break;
                    case 'tomorrow':
                        $text = __('naekb.tgbot::commands.group.tomorrow') . ' (' . Carbon::now()->addDay()->format('d.m.Y H:i') . ')' . PHP_EOL . PHP_EOL;
                        $day = Carbon::now()->addDay()->dayOfWeek;
                        break;
                    case 'after':
                        $text = __('naekb.tgbot::commands.group.after') . ' (' . Carbon::now()->addDays(2)->format('d.m.Y H:i') . ')' . PHP_EOL . PHP_EOL;
                        $day = Carbon::now()->addDays(2)->dayOfWeek;
                        break;
                    default:
                        $text = __('naekb.tgbot::commands.group.error');
                        break;
                }
            }
