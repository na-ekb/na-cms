...
class MemberMenuCommand extends Command
{
    ...
    public function __construct() {
        $this->description = __('naekb.tgbot::commands.member.description');
    }
    ...
    public function handle(array $arguments = [])
    {
        ...
        if (!empty(config('primary.primary_city'))) {
            ...
            $keyboard[] = [
                Keyboard::button([
                    'text'          => __('naekb.tgbot::commands.member.groups_in') . " {$where}",
                    'callback_data' => 'groups  0'
                ]),
                Keyboard::button([
                    'text'          => __('naekb.tgbot::commands.member.groups_region'),
            ...
            $keyboard[] = [
                Keyboard::button([
                    'text'          => __('naekb.tgbot::commands.member.groups'),
                    'callback_data' => 'groups  0'
                ])
            ];
        }
        ...
        $keyboard[1][] = Keyboard::button([
            'text'          => __('naekb.tgbot::commands.member.clean_time'),
            'callback_data' => 'cleanTime'
        ]);

        $keyboard[2][] = Keyboard::button([
            'text'          => __('naekb.tgbot::commands.member.jft'),
            'callback_data' => 'jft'
        ]);
        ...
        if (!empty(config('TgBot.tg_channel'))) {
            ...
            if ($chatMember == false) {
                $keyboard[$row][] = Keyboard::button([
                    'text'          => __('naekb.tgbot::commands.member.channel'),
                    'callback_data' => 'channel  addToChannel'
                ]);
            } else {
                $keyboard[$row][] = Keyboard::button([
                    'text'          => __('naekb.tgbot::commands.member.channel_post'),
        ...
        if (!empty(config('TgBot.tg_channel')) && AdminCheck::isAdmin(config('TgBot.tg_channel'), $user_id)) {
            $keyboard[$row][] = Keyboard::button([
                'text'          => __('naekb.tgbot::commands.member.admin'),
                'callback_data' => 'admin'
            ]);
            $row++;
        }
