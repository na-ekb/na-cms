...
use NAEkb\TgBot\Entities\TgCleanDate;
use NAEkb\TgBot\Entities\TgState;
...
class CleanTimeCommand extends AbstractCommand
{
    ...
    {
        if (TgCleanDate::where('user_id', $this->userId)->exists()) {
            $this->start();
        } else {
            $this->set(true);
        }
    ...
    protected function start() {
        $from = TgCleanDate::where('user_id', $this->userId)->first()->date;
        ...
        if($days > 0) {
            if ($months > 0 || $years > 0) {
                $date .= ' ' . __('naekb.tgbot::commands.and');
            }
            if (app()->getLocale() == 'ru') {
        ...
        if (empty($date)) {
            $date = 'naekb.tgbot::commands.clean_time.jft';
        }

        $text = __('naekb.tgbot::commands.clean_time.clean', [
            'date' => $date
        ]);
        ...
        $keyboard = [
            [
                Keyboard::button([
                    'text'          => __('naekb.tgbot::commands.clean_time.delete'),
    ...
    protected function set(bool $first = false)
    {
        ...
        if (empty(config('TgBot.tg_channel'))) {
            $text = __('naekb.tgbot::commands.clean_time.set');
        } else {
            $text = __('naekb.tgbot::commands.clean_time.set_channel');
        }

        $keyboard = [
            [
                Keyboard::button([
                    'text'      => __('naekb.tgbot::commands.clean_time.set_btn'),
    ...
    protected function parse(array $arguments = [])
    {
        ...
        if (!empty($text)) {
            try {
                ...
                if (empty(config('TgBot.tg_channel'))) {
                    $text = __('naekb.tgbot::commands.clean_time.setted');
                } else {
                    $text = __('naekb.tgbot::commands.clean_time.setted_channel');
                }
            } catch (\Throwable $e) {
                $text = __('naekb.tgbot::commands.clean_time.error');
            }
        }
    ...
    protected function del(array $arguments = [])
    {
        ...
        $user_id = $this->getUpdate()->getMessage()->getChat()->getId();
        TgCleanDate::where('user_id', $user_id)->delete();

        $this->replyWithMessage([
            'text' => $arguments['empathy'] ? __('naekb.tgbot::commands.clean_time.empathy') : __('naekb.tgbot::commands.clean_time.deleted'),
            'reply_markup' => new Keyboard([
                'inline_keyboard' => [
                    [
                        Keyboard::button([
                            'text'          => __('naekb.tgbot::commands.clean_time.set_again'),
    ...
    protected function congr(array $arguments = []) {
        ...
        if (!empty(config('TgBot.tg_channel')) && !$arguments['private']) {
            ...
            if (!empty($months->clean)) {
                ...
                $this->telegram->sendMessage([
                    ...
                    'text'                      => __('naekb.tgbot::commands.channel.congr', [
                        'userId'    => $user->getId(),
                        'userName'  => $name,
                        'date'      => $text
                    ]),
        ...
        $this->replyWithMessage([
            'text' => $arguments['private'] ? __('naekb.tgbot::commands.clean_time.congr') : __('naekb.tgbot::commands.clean_time.congr_channel'),
            'reply_markup' => new Keyboard([
                'inline_keyboard' => [
                    [
                        Keyboard::button([
