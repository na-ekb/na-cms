...
oc.registerControl('custom-select', class extends oc.ControlBase {
    ...
    bindCustomSelect() {
        ...
        if (this.element.dataset.token && this.element.dataset.url) {
            options = this.buildAjaxHandlerOption(options, this.element.dataset.token, this.element.dataset.url);
        }
    ...
    buildAjaxHandlerOption(selectOptions, token, url) {
        var self = this;

        selectOptions.ajax = {
            transport: function(params, success, failure) {
                if (!params.data.term || params.data.term <= 3) {
                    return [];
                }
                var request = $.ajax({
                    type: "POST",
            ...
            processResults: function (data, params) {
                var results = data.suggestions || data.suggestions,
                    options = [];
...
addEventListener('render', function() {
    document.querySelectorAll('select.dadata-select').forEach(function(element) {
